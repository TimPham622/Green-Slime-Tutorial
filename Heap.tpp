...
public:
    ...
    void insert(T element)
    {
        ...
        while (currentIndex > 1) {
            ...
            if(this->tree.at(parentIndex) > this->tree.at(currentIndex)) {

                std::cout << "Swap positions " << parentIndex << "(" << this->tree.at(parentIndex)
                 << ") and " << currentIndex << "(" << this->tree.at(currentIndex) << ")." << std::endl;
                std::swap(this->tree.at(parentIndex), this->tree.at(currentIndex));
    ...
    void remove(T value)
    {
        ...
        this-> tree.at(targetIndex) = this -> tree.back();
        this->tree.pop_back();

        if (targetIndex >= this->tree.size()) {
            return;
        }
         heapIndex parentIndex = this->getParentPosition(targetIndex);

        if (targetIndex > 1 && this->tree.at(targetIndex) < this->tree.at(parentIndex)) {
            heapIndex currentIndex = targetIndex;
            while (currentIndex > 1) {
